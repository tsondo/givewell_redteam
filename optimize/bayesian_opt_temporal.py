"""GP-based Bayesian optimization for 10-dimensional temporal ITN CEA.

Extends Phase 1's 5-parameter optimizer with distribution interval,
logistics fraction, and three decay time constants (tau_physical,
tau_insecticide, tau_usage).
"""
...
def _x_to_params(x: np.ndarray) -> dict[str, float]:
    """Convert a raw parameter vector to the dict evaluate_temporal_cea expects."""
...
def optimize_country_temporal(
    ...
    """Run Bayesian optimization for one country in one direction (temporal model).
    ...
    # Baseline evaluation (temporal model at default params)
    ...
    if verbose:
        print(f"         after Sobol: best $/DALY = {best_y:.2f}")
